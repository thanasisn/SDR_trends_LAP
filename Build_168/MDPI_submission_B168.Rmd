...
cleartrend <- signif(LT[DATA == "Clear sky cond.",  slope], 1)
...
trend is much smaller (\~$`r cloudtrend`\,\%/y$). 
...
in cloud cover and/or optical depth. 
...
For SZAs $25^\circ$ -- $75^\circ$,
the trends are variable and for most SZAs insignificant ranging between $0$ and $0.2\,\%/y$.
The smooth increase of the trend at SZAs larger than $\sim 75^\circ$ is probably associated with stronger attenuation by
...
spp$cor.estimate   <- signif(spp$cor.estimate,   3)
spp$RsqrdAdj       <- NULL
spp$Rsqrd          <- NULL
spp$cor.t          <- NULL
...
spp <- rename(spp, "Pearson cor."   = cor.estimate)
...
a value close to the long term trend) both statistically significant at the $99\,\%$ confidence level.
