...
affiliation:
  - num: 1
    ...
    email: natsisphysicist@gmail.com (A.N.); abais@auth.gr (A.B.)
  - num: 2
    address: |
      Your department
      Street, City, Country
...
# back matter
keywords: |
  GHI; SDR; Solar Brigthening/Dimming, aerosols, clouds.
...
<!-- goals -->
We reevaluated and extended the dataset used by @Bais2013, we applied a different algorithm for the identification of clear-/cloud-sky instances [@Reno2016; @Reno2012a], and we derived the SDR trends for the period `r year(min(ALL_1_daily_DESEAS$Date))` to `r year(max(ALL_1_daily_DESEAS$Date))`, under different sky conditions (all-sky, clear-sky and cloud-sky).
...
```{r trendALL, echo=F, out.width='.70\\linewidth', fig.cap=paste0("Anomaly (\\%) of the daily all-sky SDR, relative to climatological values for ", year(min(ALL_1_daily_DESEAS$Date)), " - ", year(max(ALL_1_daily_DESEAS$Date)), ". The black line shows the long term linear trend.")}
...
```{r cusummonth, echo = FALSE, fig.cap='Running cumulative sum plots of the monthly SDR.', fig.subcap=c('All-skies.', 'Clear-skies.', 'Cloud-skies.'), out.width='.32\\linewidth', fig.asp=1, fig.ncol = 3}
...
```{r cusumnotrendmonthly, echo = FALSE, fig.cap='Running cumulative sum plots of the monthly SDR after removing the long term trend from the monthly values.', fig.subcap=c('All-skies.', 'Clear-skies.', 'Cloud-skies.'), out.width='.32\\linewidth', fig.asp=1, fig.ncol = 3}
knitr::include_graphics(c(
...
```{r seasonalALL, echo=F, fig.cap="Linear trends (black lines) of monthly mean SDR season (columns of graphs) for the three sky conditions (rows of graphs). The vertical scale is common for all graphs.", out.width="100%"}
