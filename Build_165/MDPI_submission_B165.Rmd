...
The "dark signal" is calculated by averaging all measurements recorded for a period of $3\,\text{h}$, before (morning) or after (evening) the Sun reaches an elevation angle of $-10^\circ$.
...
$`r FIBais_Az_1`^\circ$ and $`r FIBais_Az_2`^\circ$ with SZA greater than $`r 90 -  FIBais_Elev`^\circ$.
...
a) we accept only days with more than `r 100 * All_daily_ratio_lim`% of the daytime measurements present and valid,
b) on the dataset of clear- and cloudy-skies, we included only days with more than
`r 100 * Clear_daily_ratio_lim`% of the daytime measurements identified as clear or cloudy respectively,
c) monthly values were computed from daily means
...
LT$slope    <- signif(LT$slope,   4)
LT$slope.p  <- signif(LT$slope.p, 4)
LT$Rsqrd    <- signif(LT$Rsqrd,   4)
