...
abstract: |
  The shortwave downward solar irradiance (SDR).........
  Human and climate.......
  Brigthening dimming...............
  We study 29 years of CHP1 data.........
  Clear cloud id ................
  Found trends ......
keywords: 
  - GHI
...
In Figure\ \ref{fig:trendALL} we present only the time series under all-sky conditions; the plots for clear-sky and cloud-sky conditions, are very similar.
...
$`r format(TSIslopeAll[1,1], scientific = T, digits = 2)`\%/year$, and thus we can not attribute any major effect on SDR trend to Sun variability.
...
```{r trendALL, echo=F, out.width='.70\\linewidth', fig.cap=paste0("Anomaly (\\%) of the daily All-sky SDR, relative to climatological values for ", year(min(ALL_1_daily_DESEAS$Date)), " - ", year(max(ALL_1_daily_DESEAS$Date)), ". The black line shows the long term trend for all-sky conditions.")}
knitr::include_graphics("./images/LongtermTrends-2.png")
```


Although the trends are quite constant, using plots of the cumulative sums we can reveal some structure within the trends and some differences between the sky conditions [@Regier2019]. 
On all three plots on Figure\ \ref{fig:cumsum}, we observe a downward part at the start of the period until about 2000, an almost steady part until about 2016 for All-sky and clear-sky conditions (Figures\ \ref{fig:cumsum-1}, \ref{fig:cumsum-2}) and until 2009 for cloud-sky conditions (Figure\ \ref{fig:cumsum-3}), and finally, a steep upward part until the present.
...
Another distinct feature of the cumulative plots, is the difference, between the cloud-sky and the rest of the sky conditions, where they display a supplementary behavior. 
...
Although there is a seasonal variation of the minimum SZA, that is not examined here.
The brightening effect of SDR is stronger for All-sky and clear-sky condition for low Sun elevation angles (Figures\ \ref{fig:szatrends}a and \ref{fig:szatrends}b).
In the case of cloud-sky condition (Figure\ \ref{fig:szatrends}c), there is no strong correlation between SDR brightening and SZA, except for some low Sun angles, that occur only for a small part of the year, and thus we can not draw more conclusions from this analysis for the overall trends.
...
Similar to the long term trends discussed above, we have produced the deseasonalized trend for the different sky conditions and for each season of the year, using the corresponding mean monthly values
(Figure\ \ref{fig:seasonalALL} and Table\ \ref{tab:trendseasontable}).
For All-sky condition the brightening of SDR is very significant for the Winter, close to the long term trend for the Autumn, and almost four times lower than the long term trend for Sprint and Summer, indicating a relation to the cloud pattern of each season.
...
```{r seasonalALL, echo=F, fig.cap="SDR trends by season of the year for the three sky conditions. Each point is a monthly mean calculated from daily means. The vertical scale is common for all subplots, for easier comparison.", out.width="100%"}
...
pp$var                <- NULL
rownames(pp)          <- NULL
names(pp) <- c("Trend [%/year]", "Trend p-value", "Sky condition", "Season", "Statistical signif. [%]")
## order by season
target    <- c("Winter", "Spring", "Summer", "Autumn")
new_order <- sapply(target, function(x,pp){which(pp$Season == x)}, pp = pp)
pp        <- pp[new_order,]
# ## put all sky first
# A  <- which(pp$`Sky condition` == "All sky cond.")
# B  <- which(pp$`Sky condition` != "All sky cond.")
# pp <- pp[c(A,B),]
## numeric precision
pp$`Trend [%/year]`          <- signif(pp$`Trend [%/year]`, 3)
pp$`Trend p-value`           <- signif(pp$`Trend p-value`,  3)
pp$`Statistical signif. [%]` <- signif(pp$`Statistical signif. [%]`,  3)
...
panderOptions("table.alignment.default", "center")
pander(pp,
       justify = c('right', 'right', 'right', 'right', 'center'),
       split.table = Inf,
       caption = "\\label{tab:trendseasontable}SDR trends of monthly anomaly for each season of the year.")
```
<!-- \normalsize -->
...
`r paste(year(min(ALL_1_daily_DESEAS$Date)), " - ", year(max(ALL_1_daily_DESEAS$Date)))`, there is a brightening trend of SDR for All-sky conditions ($`r alltrend`\%/year$).
On a previous study [@Bais2013], for the period 1993 - 2011 the trend was found to be $0.33\%/year$.
The increase of this trend, indicates that the phenomenon and probably the causes, are still occurring.
Accordingly, we found a similar trend for clear-sky conditions ($`r cleartrend`\%/year$) that further supports that the brightening can be independent of the cloud occurrences. 
...
The dimming effect of SDR under cloud-sky condition ($`r cloudtrend`\%/year$), shows that clouds can also attenuate the amount of SDR reaching the ground, and changes on their occurrences can effect the net energy balance.
...
The observed brightening of SDR over Thessaloniki, is correlated with the SZA (higher SZA, have stronger brightening trends). And also, with the seasons of the year, where Winter and Autumn show a significant strong trend, in contrast to Spring and Summer.

Our finds are in agreement with other researchers for the region.
Future observation from the colocated pyrheliometer, may provide us with the means to further investigate the variability of Sun radiation on ground level in Thessaloniki. 
Also, more data (cloud occurrence, aerosols, atmospheric water, etc.), will help us to determine the extent of SDR attenuation by each factor. 
...
```{r trendCLEAR, echo=F, fig.cap=paste0("Anomaly (\\%) of the daily clear-sky SDR, relative to climatological values for", year(min(ALL_1_daily_DESEAS$Date)), " - ", year(max(ALL_1_daily_DESEAS$Date)), ". The black line shows the long term trend for clear-sky conditions.")}
knitr::include_graphics("./images/LongtermTrends-5.png")
```

```{r trendCLOUD, echo=F, fig.cap=paste0("Anomaly (\\%) of the daily cloud-sky SDR, relative to climatological values for ", year(min(ALL_1_daily_DESEAS$Date)), " - ", year(max(ALL_1_daily_DESEAS$Date)), ". The black line shows the long term trend for cloud-sky conditions.")}
