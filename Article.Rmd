...
As a reference clear sky SDR, we are using the radiation model by Haurwitz’s, adjusted for our site with the factor $a = `r signif(alpha,3)`$ (Eq.\ \ref{eq:ahau}), resulted by an iterative optimization process, as described by @Long2000 and @Reno2016.
The target of the optimization was the minimization of the function $f(a)$ (Eq.\ \ref{eq:minf}) and was accomplished with the computing function `stats::optimise` [@RCT2023], which is based on work by @Brent1973.
