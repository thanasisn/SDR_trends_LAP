...
The TSI data we use, comes from NOAA [@Coddington2005], and were interpolated to match with the time step of our measurements. 
The final dataset contains
$`r SDR_N_All`$
one-minute measurements, of which,  $`r signif(100 * SDR_N_Clear / SDR_N_All, 3)`\%$
correspond to clear-sky conditions and 
$`r signif(100 * SDR_N_Cloud / SDR_N_All, 3)`\%$
to cloud-sky conditions.
...
b) accordingly, monthly values were computed from daily means
...
To estimate SZA contribution to the SDR trends, the one-minute data were aggregated the data in $1^\circ$ SZA bins, separately for the morning and afternoon hours, which were then deseasonalized as mentioned above.



a) for daily mean values we excluded days with less
than 
[This is not a good criterion! It must be seasonal and 2 hours of
data are not enough for the calculation of daily means.\
How about: "excluded days with more than 180 missing i-minute values
during daylight hours".]{.comment-start id="6" author="Alkis Bais"
date="2023-07-14T07:52:00Z"}180 valid data points[]{.comment-end
id="6"}, 
