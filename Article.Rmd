...
## no need all are 100%
LTminSSig <- signif(min(LT$`Trend statistical signif. [%]`),4)
...
In order to compare any features within the trends, we created another set of CUSUMs plots, by subtracting the corresponding long term trend from the data, prior the CUSUM calculation (Figure\ \ref{fig:cumsumnotrend}).
With this approach periods where the CUSUM diverge from zero can be interpreted as systematic variation of SDR, further from the climatological mean.

.......
cusums without trends?
........
...
For cloud-sky conditions (Figure\ \ref{fig:szatrends}c),there is no dependence of the trends with SZA. 

The trends are variable ranging within about $\pm 0.2\,\%/y$. 

Between $55^\circ$ and $75^\circ$ SZA the trends are mostly negative for cloud sky conditions and are possibly associated with stronger attenuation by clouds. 
...
```{r  echo=F, fig.cap="Seasonal SZA", out.width='.32\\linewidth'}
