...
where: $n$ is the total number of daylight records, $\text{SDR}_{\text{CSid},i}$ are the records identified as clear sky by CSid, $a$ is a hypothetical adjustment factor, and $\text{SDR}_{\text{testCSref},i}$ is the tested model of clear sky radiation.
...
b) Maximum measured value $M_{\text{}}$, and maximum reference $M_{\text{CSref}}$ (Eq.\ \ref{eq:MaxVIP}).
