...
## numeric precision
spp$slope          <- signif(spp$slope,          3)
spp$slope.p        <- signif(spp$slope.p,        3)
...
For clear-skies, the trend in winter is $`r signif(spp$"Trend [%/year]"[spp$Season == "Winter" & spp$"Sky condition" == "Clear skies"], 2)`\,\%/y$, larger than for all-skies ($`r signif(spp$"Trend [%/year]"[spp$Season == "Winter" & spp$"Sky condition" == "All skies"], 2)`\,\%/y$), which is another indication of a decreasing trend in cloud optical thickness.
...
and
$`r signif(spp$"Trend [%/year]"[spp$Season == "Autumn" & spp$"Sky condition" == "Clear skies"], 2)`\,\%/y$
...
```{r SZAtrendSeason, echo = FALSE, fig.cap='Seasonal distribution of the SDR\'s long term trends by SZA before and after noon. The solid data points represent values with an acceptable statistical significance ($p<0.005$), the trends were computed with SZA bins of $1^\\circ$ for each day, before and after local noon, all graphs have the same scale. Bins with $p<0.005$ or with less than 85 data points may be missing from the view.', out.width='100%'}
knitr::include_graphics("./images/SzaTrendsSeasTogether-2.png")
```

<!-- ```{r echo = FALSE, fig.cap='*** Montly *** SDR trend ~ SZA', out.width='100%'} -->
<!-- knitr::include_graphics("./images/SzaTrendsSeasTogetherMonthly-2.png") -->
<!-- ``` -->
