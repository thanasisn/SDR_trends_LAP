...
We demonstrate that in the period `r paste(year(min(ALL_1_daily_DESEAS$Date)), " - ", year(max(ALL_1_daily_DESEAS$Date)))`, there is brightening trend of SDR for All-sky conditions ($`r alltrend`\%/year$).
...
We have calculate the effect of TSI variability to be negligible, but unfortunately there is no available data for this period for the Aerosols, in order to estimate their effect on SDR, which is the most probable main cause.
