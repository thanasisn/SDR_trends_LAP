...
Overall, for all- and clear-sky conditions we observe that there are periods where the anomalies diverge from the expected climatological value, that each lasts for few only years, and then diverge to the opposite site, creating a variation around zero.
It is interesting that in the period 1993 - 2016 the anomalies of all-sky and clear sky, have a high variability around zero and after 2016, the range of the variability is about one third of the prior period.
Even more strong fluctuations exist for cloud-sky conditions, where there are long period with relative low variation, and few periods with very strong divergences. We can identify two very positive periods (1997 - 2005 and 2020 - 2023) and one very negative (2005 - 2010).
...
<!--
```{r cusumALL, echo=F, fig.cap="Running cumulative sum."}
knitr::include_graphics("./images/CumulativeDailyCuSum-1.png")
```
-->

<!--
Not showing this trend
```{r cusumCLEAR, echo=F, fig.cap="Donditions.", include=FALSE}
knitr::include_graphics("./images/CumulativeDailyCuSum-5.png")
```
-->

<!--
```{r cusumCLOUD, echo=F, fig.cap="Donditions."}
knitr::include_graphics("./images/CumulativeDailyCuSum-9.png")
```
-->
...
```{r echo = FALSE, fig.cap='Distribu.', fig.subcap=c('', '', ''), out.width='.32\\linewidth', fig.asp=1, fig.ncol = 3}
