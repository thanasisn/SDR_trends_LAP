...
Before further analysis, we implement an appropriate aggregation scheme on the 1-minute data, to preserve the ....representabiliyt....
When we use daily mean values we exclude instances with less than `r Daily_aggregation_N_lim` valid data points.
Accordingly, monthly values are computed from the daily aggregated data, where months with less than `r Monthly_aggegation_N_lim` days are rejected. 
The daily seasonal values are indexed based on the day of year number, and the monthly by the corresponding calendar month.


<!-- Seasonal -->
For the seasons of the year trends, we also grouped the mean daily values by season (December - February: Winter, March - May: Spring, etc.)
...
These values are presented in the following section.
...
Significance and aggregation scheme and filtering will be noted along with the corresponding results.
...
```{r trendALL, echo=F, fig.cap="Anomaly (\\%) of the daily Global radiation relative to climatological values for 1993 - 2023. The straight black line shows the long term trend for All-sky conditions."}
